...
const processRequest = async (options) => {
    const { contextFactory, execute = graphql_1.execute, formatPayload = ({ payload }) => payload, operationName, extensions, parse = graphql_1.parse, query, request, rootValueFactory, schema, subscribe = graphql_1.subscribe, validate = graphql_1.validate, validationRules, variables, allowedSubscriptionHttpMethods = ["GET", "POST"], } = options;
    let context;
    let rootValue;
    let document;
    let operation;
        ...
        try {
            ...
            try {
                ...
                if (operation.operation === "subscription") {
                    if (!allowedSubscriptionHttpMethods.some((method) => (0, index_1.isHttpMethod)(method, request.method))) {
                        throw new errors_1.HttpError(405, `Can only perform subscription operation from a ${allowedSubscriptionHttpMethods.join(" or ")} request.`, {
                            headers: [
